...
#### have not figure out NREL  IP ranges... allowing 0.0.0.0 for now

resource "aws_security_group" "tf-sg-all" {
  #name = "secgrp-by-tf"
  #name_prefix = "tf4sg-"   # end up with name like tf4sg-20220809210640999900000001 for all ingress rules
  name_prefix = "tf4sgall-"   # end up with name like tf4sg-20220809210640999900000001 for all ingress rules
  ...
  ingress {
    ...
    to_port     = 30999
    protocol    = "udp"
    cidr_blocks = [ 	"20.22.9.18/32" ]   # represent be a last change date...
  }
